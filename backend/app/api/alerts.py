...
@router.get("/active")
async def get_active_risks(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Risk).where(Risk.status == "open"))
    return result.scalars().all()


@router.get("/project/{project_id}")
async def get_project_alerts(project_id: int, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
