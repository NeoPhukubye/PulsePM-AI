...
@router.get("/today")
async def get_today_standup(db: AsyncSession = Depends(get_db)):
    agent = StandupAgent()
...
@router.get("/history")
async def get_standup_history(db: AsyncSession = Depends(get_db)):
...
async def generate_standup(project_id: int = None, db: AsyncSession = Depends(get_db)):
    agent = StandupAgent()
